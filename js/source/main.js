...
function Model(modelName) {
  ...
  // Parses localStorage data as JSON and caches it in a variable
  var localStorageContent = JSON.parse(localStorage.getItem(this.modelName));
 ...
 * @returns {int}
 */
Model.prototype.getObjectIndex = function (id) {
  'use strict';

  // Creates a map of IDs
  var objectMap = this.objectArray.map(function (object) {
    return object.id;
  });
...
Model.prototype.save = function () {
  ...
  var stringifiedJSON = JSON.stringify(this.objectArray);
...
Model.prototype.get = function (id) {
  ...
  var objectIndex = this.getObjectIndex(id);
 ...
 */
Model.prototype.set = function (objects) {
  'use strict';
 ...
 */
Model.prototype.modify = function (id, object) {
  'use strict';
...
function ToDo() {
  ...
  // TODO: Make these variables customisable by passing in an options object into the constructor
 ...
 * @returns {string}
 */
ToDo.prototype.getTaskTemplate = function (name, id, done) {
  'use strict';
 ...
 * @param {boolean} done - Whether the task is done or not
 */
ToDo.prototype.setTask = function (name, id, done) {
  var taskHTML = this.getTaskTemplate(name, id, done);
 ...
 */
ToDo.prototype.todoController = function () {
  'use strict';
 ...
 */
ToDo.prototype.taskController = function () {
  'use strict';

  // Changes the done state of a task
  var changeStatus = (function (task, done) {
    var taskInput = task.querySelector('[data-target="' + this.taskInputTarget + '"]');
    var taskID = parseInt(task.getAttribute('data-id'), 10);
  ...
  var toggleEdit = (function (task) {
    var taskInput = task.querySelector('[data-target="' + this.taskInputTarget + '"]');
    var taskEdit = task.querySelector('[data-target="' + this.taskEditTarget + '"]');
    var taskValue = taskInput.value;
    var taskID = parseInt(task.getAttribute('data-id'));
  ...
  var drag = dragula([this.todoListElement, this.doneListElement]);

  // Dragula's drop event listener
  // TODO: Change ordering in model on drag and drop
  drag.on('drop', (function (el, target) {
    var taskID = parseInt(el.getAttribute('data-id'));
    var taskCheckbox = el.querySelector('[data-target="task-checkbox"]');
    var modifyTask = (function (taskID, done) {
